...
void populateCommands(char *, int, int, char** );
int PATH_MAX = 100;
...
void sigintHandler(int sig_num)
{
  ...
  kill(pid, SIGTERM);
  sleep(10);
  pid_t waitId = waitpid(pid, &status, WNOHANG); //returns immediately if no child has returned.
  //waitId is 0 if no child has returned.
  if (waitId == 0)
  {
    kill(pid, SIGKILL);
  }
  waitpid(pid, &status, 0); //harvests the zombie function
  printf("Exiting the program\n");
  tokenizer(curr_command);
  //exit(1);
}

int tokenizer(char * curr_command){
  char cwd[PATH_MAX];
  curr_command = malloc(sizeof(char) * 200);
